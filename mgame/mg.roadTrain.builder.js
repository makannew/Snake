...
function buildRoadTrain({ wheelsInfo }) {
  ...
  for (let wheelInfo of roadTrain.wheelsInfo) {
    ...
    let wheelOutPos = new THREE.Vector3(
      wheelInfo.doubleWheelGap / 2 +
        (wheelInfo.width - wheelInfo.doubleWheelGap) / 4,
      0,
      0
    );
    let wheelInPos = new THREE.Vector3(
      wheelInfo.doubleWheelGap / 2 +
        (wheelInfo.width - wheelInfo.doubleWheelGap) / 4,
      0,
      0
    ...
    let wheelPos = new THREE.Vector3(
      wheelInfo.axelLength,
      wheelInfo.radius - wheelInfo.axelHeight,
      wheelInfo.distance
    );
    if (wheelInfo.left) {
      rotation = Math.PI / 2;
    ...
    let correctionQuat = new THREE.Quaternion().setFromAxisAngle(
      new THREE.Vector3(0, 0, 1).normalize(),
      rotation
    );
    wheel.correctionQuat = new THREE.Quaternion(
      correctionQuat.x,
      correctionQuat.y,
      correctionQuat.z,
      correctionQuat.w
    );
    let wheelQuat = new THREE.Quaternion(
      threeQuat.x,
      threeQuat.y,
      threeQuat.z,
      threeQuat.w
    ...
    let radius = wheelInfo.doubleWheelGap
      ? wheelInfo.axelDiameter
      : wheelInfo.radius;
    ...
    if (wheelInfo.doubleWheelGap) {
      ...
      wheel.wheelIn.set({
        geometryName: "cylinder",
        dimension: {
          radiusTop: wheelInfo.radius,
          radiusBottom: wheelInfo.radius,
          height: (wheelInfo.width - wheelInfo.doubleWheelGap) / 2,
        },
    ...
    let susLength =
      (wheelInfo.axelLength - wheelInfo.width / 2) * roadTrain.gapRatio;
    ...
    let susPos = new THREE.Vector3(
      wheelInfo.axelLength - wheelInfo.width / 2 - susLength / 2,
      0,
      wheelInfo.distance
    );
    ...
    suspension.set({
      geometryName: "box",
      dimension: {
        height: wheelInfo.axelDiameter,
        width: wheelInfo.axelDiameter,
        length: susLength,
      },
  ...
  roadTrain.chassisLength =
    roadTrain.chassisFrontLength - roadTrain.chassisRearLength;
  if (Math.abs(chassisFrontLength) < Math.abs(chassisRearLength)) {
    ...
    extensionChassisLocalZ =
      -roadTrain.chassisFrontLength +
      (roadTrain.chassisRearLength + roadTrain.chassisFrontLength) / 2;
  } else {
    massCenterChassisLength = Math.abs(roadTrain.chassisRearLength) * 2;
    extensionChassisLocalZ =
      -roadTrain.chassisRearLength +
      (roadTrain.chassisRearLength + roadTrain.chassisFrontLength) / 2;
  }
  ...
  roadTrain.chassisTop.set({
    geometryName: "box",
    dimension: {
      height: roadTrain.chassisLength,
      width: roadTrain.chassisTickness,
      length: roadTrain.chassisWidth,
    },
    position: {
      x: x + chassisTopPos.x,
      y: y + chassisTopPos.y,
  ...
  let chassisPos = new THREE.Vector3(0, -roadTrain.axelsVerticalFreedom, 0);
  chassisPos.applyQuaternion(threeQuat);

  roadTrain.chassis.set({
    geometryName: "box",
    dimension: {
      height: massCenterChassisLength,
      width: roadTrain.chassisTickness,
      length: roadTrain.chassisWidth,
    },
  ...
  if (extensionChassisLength != 0) {
    ...
    roadTrain.chassisExtension.set({
      geometryName: "box",
      dimension: {
        height: extensionChassisLength,
        width: roadTrain.chassisTickness,
        length: roadTrain.chassisWidth,
      },
      position: {
        x: x + chassisExtensionPos.x,
        y: y + chassisExtensionPos.y,
        z: z + chassisExtensionPos.z,
      },
      quaternion: new THREE.Quaternion(
        threeQuat.x,
        threeQuat.y,
  ...
  if (roadTrain.cabinInfo) {
    for (let cabin of cabinInfo) {
      ...
      let cabinLocalPos = new THREE.Vector3(
        cabin.localPosition.x,
        cabin.localPosition.y,
        cabin.localPosition.z
      );
      let cabinLocalQuat = new THREE.Quaternion(
        cabin.localQuaternion.x,
        cabin.localQuaternion.y,
        cabin.localQuaternion.z,
        cabin.localQuaternion.w
      );
      cabinLocalPos.applyQuaternion(threeQuat);
      cabinLocalPos.add(new THREE.Vector3(x, y, z));
      cabinLocalQuat.multiply(threeQuat);
      cabinPart.set({
        position: cabinLocalPos,
        quaternion: cabinLocalQuat,
        visible: false,
        sleep: true,
      });
  ...
  for (let i = 0, len = roadTrain.wheels.length; i < len; ++i) {
    ...
    roadTrain.addLink(
      roadTrain.suspensions[i].body,
      roadTrain.suspensionsBodies[i]
    );
  }
  ...
  function updateFromChassis({ chassisPos, chassisQuat, mesh, timeStamp }) {
    ...
    let chassisPosThree = new THREE.Vector3(
      chassisPos.x,
      chassisPos.y,
      chassisPos.z
    );
    let chassisQuatThree = new THREE.Quaternion(
      chassisQuat.x,
      chassisQuat.y,
      chassisQuat.z,
      chassisQuat.w
